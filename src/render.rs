...
impl CompletableRenderData for NoteAndProfileRenderData {
    fn is_complete(&self) -> bool {
        self.note_rd.is_complete() && self.profile_rd.as_ref().map(|prd| prd.is_complete()).unwrap_or(true)
    }
    ...
    fn is_complete(&self) -> bool {
        match self {
            RenderData::Profile(profile_rd, notes_rd) => {
                profile_rd.as_ref().map(|prd| prd.is_complete()).unwrap_or(true)
    ...
    fn nostrdb_filters_to_complete(&self) -> Vec<nostrdb::Filter> {
        ...
        match self {
            RenderData::Profile(profile_render_data, notes_render_data) => {
                ...
                filters.extend(notes_render_data.iter().flat_map(|nrd| nrd.nostrdb_filters_to_complete()));
            },
...
impl RenderData {
    ...
    pub fn profile(profile_rd: Option<ProfileRenderData>, notes_rd: Vec<NoteAndProfileRenderData>) -> Self {
...
fn get_user_profile_render_data_from(pubkey: &[u8; 32], ndb: &Ndb, txn: &Transaction) -> Result<RenderData> {
    ...
    for query_result in notes_query_results {
        ...
        notes_rd.push(NoteAndProfileRenderData::new(note_rd, Some(note_author_profile_rd)));
    }
