...
pub fn note_render_data_to_json(
    ...
    let note = ndb.get_note_by_key(&txn, note_key)
...
pub fn note_json_html(
    note_json: &Value
) -> Result<String, Error> {
    let reg = Handlebars::new();

    let template_contents = std::fs::read_to_string("./html_templates/note.hbs")
        .map_err(|e| {
            error!("Failed to read profile template: {:?}", e);
            Error::CantRender
        })?;

    let parsed_profile_content = serde_json::from_str::<Value>(&note_json["profile"]["content"].as_str().unwrap())?;

    let display_name = if parsed_profile_content["display_name"].as_str().unwrap_or_default().is_empty() {
        parsed_profile_content["name"].as_str().unwrap_or("nostrich")
    } else {
    ...
    let page_contents = reg.render_template(&template_contents, &note_json).map_err(|e| {
