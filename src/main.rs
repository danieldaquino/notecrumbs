...
fn serve_profile_html(
    ...
    let template_css_contents = std::fs::read_to_string("./html_templates/output.css")
        .map_err(|e| {
            error!("Failed to read profile template CSS: {:?}", e);
            Error::CantRender
        })?;

    let template_contents = std::fs::read_to_string("./html_templates/profile.hbs")
        .map_err(|e| {
    ...
    let notes_html_items: Vec<String> = notes_rd.iter().filter_map(|note_rd| {
